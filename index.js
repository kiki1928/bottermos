...
client.on('qr', qr => {
   ...
   console.log(`[ ${time} ] QR code is ready, scan Bang `)
})
...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["+62 882-9203-0664@s.whatsapp.net"] // ganti nomer lu
           ...
           case 'demote':
            ...
            client.sendPtt(from, './media/Yamete.mp3')
              ...
              case 'pota':
              case 'Pota':
            client.reply (from, '*potaaa*', id)
            client.sendPtt(from, './media/pota.mp3')
                    break

              case 'Bisita':
              case 'bisita':
            client.reply (from, '*hari dam dam*', id)
            client.sendPtt(from, './media/bisita.mp3')
